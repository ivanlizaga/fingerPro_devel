...
#' @param completion_quantile A numeric value between 0 and 1. This parameter defines the quantile of tracers that must meet the valid `debates` target. A value of 1 (the default) means the simulation runs until all tracers reach the valud `debates` target. A value of 0.5 means the simulation stops once half of the tracers have completed the required number of valid `debates` target.
#' @param apportionment_threshold A numeric value greater than or equal to 0. This parameter sets a tolerance for the apportionment values required for a valid debate. The apportionments must fall within the range `(-threshold, 1 + threshold)`. A value of 0 (the default) enforces the condition of `0 < w_i < 1`.
...
#'
#' @export
CR2 <- function(data, debates = 1000, completion_quantile = 1.0, apportionment_threshold = 0.0, seed = 123456)
{
  ...
  while (quantile(cr1, 1.0 - completion_quantile)[[1]] < debates)
  {
    ...
    if(min(cr1[var]) < debates)
    {
      ...
      {
        win <- which.min(gof)
        if(max[win] <= (1.0 + apportionment_threshold) && min[win] >= (0.0 - apportionment_threshold))
        {
          for (i in c(1:(nsource+1)))
          {
            if(cr1[var[i]] < debates)
    ...
    setTxtProgressBar(pb, quantile(cr1, 1.0 - completion_quantile)[[1]])
  }
  
  CR_score <- 100-((cr2*100)/cr1)
  cr <- data.frame(tracer, CR_score, cr1, cr2)
  cr <- cr[rev(order(cr$CR_score)),]
  row.names(cr) <- NULL
  colnames(cr) <- c('tracer', 'CR_score', 'debates', 'debates_lost')
  cat('\n')
  cat('\n')
  return(cr)
}
